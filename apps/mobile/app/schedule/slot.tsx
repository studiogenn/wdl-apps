...
export default function SlotScreen() {
  const { routeID, selectedDate, selectedSlot, setSelectedSlot } =
    useScheduleStore();
  const { data: slots, isLoading, error } = useSlots(routeID, selectedDate);
          ...
          <View className="flex-row flex-wrap gap-3 pb-8">
            {slots.map((slot) => {
              ...
              return (
                <Pressable
                  key={slot}
                  className={`rounded-btn px-6 py-3 ${isSelected ? "bg-detergent-400" : "border-2 border-neutral-300 bg-white"}`}
