...
export function BlogSidebar({
  ...
  return (
    <aside className="space-y-6">
      <h4 className="text-lg font-bold text-primary">Featured Posts</h4>
      <ul className="space-y-4">
        {posts.map((post) => (
          <li key={post.slug} className="flex gap-3">
            ...
            <Link
              href={`/blog/${post.slug}`}
              className="font-[family-name:var(--font-poppins)] text-sm font-medium text-navy hover:text-primary"
            >
              {post.title}
            </Link>
          </li>
